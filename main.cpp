...
ReadOnlyGattCharacteristic<uint8_t> roll(0xA100, &none, GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_NOTIFY);
ReadOnlyGattCharacteristic<uint8_t> pitch(0xA110, &none, GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_NOTIFY);
ReadOnlyGattCharacteristic<uint8_t> yaw(0xA111, &none, GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_NOTIFY);
...
void change() {
  ...
  uint8_t x = (uint8_t)euler.x();
  uint8_t y = (uint8_t)euler.y();
  uint8_t z = (uint8_t)euler.z();
  ble.updateCharacteristicValue(roll.getValueHandle(), (uint8_t*)&x, sizeof(x));
  ble.updateCharacteristicValue(yaw.getValueHandle(), (uint8_t*)&y, sizeof(y));
  ble.updateCharacteristicValue(pitch.getValueHandle(), (uint8_t*)&z, sizeof(z));
...
int main()
{
  ...
  Ticker t;
  t.attach(&periodicCallback, 1);
  printf("Calibration status values: 0=uncalibrated, 3=fully calibrated\r\n");

  while(true)
  {
    if (triggerPolling && ble.getGapState().connected)
    {
